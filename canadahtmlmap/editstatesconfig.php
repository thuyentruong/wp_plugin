...
if(isset($_POST['act_type']) && $_POST['act_type'] == 'canada-html5-map-states-save') {
...
echo "<div class=\"wrap\"><h2>" . __('Configuration of Map Areas', 'canada-html5-map') . "</h2>";
?>
<script>
    ...
    jQuery(function(){
        ...
        jQuery('select[name=map_id],select[name=state_select]').change(function() {
            ...
            location.href='admin.php?page=canada-html5-map-states&map_id='+jQuery('select[name=map_id] option:selected').val()+'&s_id='+s_id;
        });
        ...
        jQuery('.stateinfo input:radio').click(function(){
            ...
            if(jQuery(this).attr('id').charAt(0)=='n'){
                ...
                jQuery("#URL"+el_id).attr("value", "javascript:canadahtml5map_set_state_text<?php echo "_$map_id" ?>("+el_id+");");
                //jQuery("#URL"+el_id).attr("readonly", false);
                jQuery("#stateURL"+el_id).fadeOut(0);
...
<form method="POST" class="canada-html5-map main" onsubmit="adjustSubmit()">
    ...
    <?php canada_html5map_plugin_nav_tabs('states', $map_id); ?>

    <p><?php echo __('This tab allows you to add the area-specific information and adjust colors of individual area on the map.', 'canada-html5-map'); ?></p>
    <p class="help"><?php echo __('* The term "area" means one of the following: region, state, country, province, county or district, depending on the particular plugin.', 'canada-html5-map'); ?></p>

    <select name="state_select">
        <option value=0><?php echo __('Select an area', 'canada-html5-map'); ?></option>
        <?php
    ...
    if($sId) {
        ...
        elseif(stripos($vals['link'], "javascript:canadahtml5map_set_state_text") === false ) $rad_other = "checked";
    ...
    <fieldset>
        <legend><?php echo __('Map area', 'canada-html5-map'); ?></legend>

        <div style="" id="stateinfo-<?php echo $vals['id']?>" class="stateinfo">
        <span class="title"><?php echo __('Name:', 'canada-html5-map'); ?> </span><input class="" type="text" name="name[<?php echo $vals['id']?>]" value="<?php echo $vals['name']?>" />
        <span class="tipsy-q" original-title="<?php esc_attr_e('Name of Area', 'canada-html5-map'); ?>">[?]</span>
        ...
        <?php echo __('do not show popup name', 'canada-html5-map'); ?>
        </label>
        <div class="clear"></div>

        <span class="title"><?php echo __('Shortname:', 'canada-html5-map'); ?> </span><input class="" type="text" name="shortname[<?php echo $vals['id']?>]" value="<?php echo $vals['shortname']?>" />
        <span class="tipsy-q" original-title="<?php esc_attr_e('Shortname of Area', 'canada-html5-map'); ?>">[?]</span>
        <div class="clear"></div>

        <span class="title"><?php echo __('What to do when the area is clicked:', 'canada-html5-map'); ?></span>
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="n<?php echo $vals['id']?>" value="nill" <?php echo $rad_nill?> >&nbsp;<?php echo __('Nothing', 'canada-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Do not react on mouse clicks', 'canada-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        <!--label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="d<?php echo $vals['id']?>" value="def" <?php echo $rad_def?> >&nbsp;<?php echo __('Show popup balloon on the map', 'canada-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Display a popup balloon with the specified information', 'canada-html5-map'); ?>">[?]</span-->&nbsp;&nbsp;&nbsp;
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="o<?php echo $vals['id']?>" value="other" <?php echo $rad_other?> >&nbsp;<?php echo __('Open a URL', 'canada-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('A click on this area opens a specified URL', 'canada-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="m<?php echo $vals['id']?>" value="more" <?php echo $rad_more?> >&nbsp;<?php echo __('Show more info', 'canada-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Displays a side-panel with additional information (contacts, addresses etc.)', 'canada-html5-map'); ?>">[?]</span><br />
        <div style="<?php echo $style_input; ?>" id="stateURL<?php echo $vals['id']?>">
            <span class="title"><?php echo __('URL:', 'canada-html5-map'); ?> </span><input style="width: 240px;" class="" type="text" name="URL[<?php echo $vals['id']?>]" id="URL<?php echo $vals['id']?>" value="<?php echo $vals['link']?>" />
            <span class="tipsy-q" original-title="<?php esc_attr_e('The landing page URL', 'canada-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
            <label><input type="checkbox" name="isNewWindow[<?php echo $vals['id']?>]" <?php if (!empty($vals['isNewWindow'])) echo 'checked="checked" '; ?>/> <?php echo __('Open url in a new window', 'canada-html5-map'); ?></label></br>
        </div>
        ...
        <span class="title"><?php echo __('Info for popup balloon:', 'canada-html5-map'); ?> <span class="tipsy-q" original-title="<?php esc_attr_e('Info for popup balloon', 'canada-html5-map'); ?>">[?]</span> </span><textarea style="width:100%; height: 150px;" class="" rows="10" cols="45" name="info[<?php echo $vals['id']?>]"><?php echo $vals['comment']?></textarea><br />
        <?php
        if(1) // just disable if((int)$tariff == 2)
        {
            ?>
            <span class="title"><?php echo __('Area color:', 'canada-html5-map'); ?> </span><input class="color colorSimple" type="text" name="color[<?php echo $vals['id']?>]" value="<?php echo $vals['color_map']?>" style="background-color: #<?php echo $vals['color_map']?>"  />
            <span class="tipsy-q" original-title='<?php esc_attr_e('The color of an area.', 'canada-html5-map'); ?>'>[?]</span><div class="colorpicker"></div>
            <label><input name="colorSimpleCh[<?php echo $vals['id']?>]" class="colorSimpleCh" type="checkbox" /> <?php echo __('Apply to all areas', 'canada-html5-map'); ?></label>
            <br />
            <span class="title"><?php echo __('Area hover color:', 'canada-html5-map'); ?> </span><input class="color colorOver" type="text" name="color_[<?php echo $vals['id']?>]" value="<?php echo $vals['color_map_over']?>" style="background-color: #<?php echo $vals['color_map_over']?>"  />
            <span class="tipsy-q" original-title='<?php echo __('The color of an area when the mouse cursor is over it.', 'canada-html5-map'); ?>'>[?]</span><div class="colorpicker"></div>
            <label><input name="colorOverCh[<?php echo $vals['id']?>]" class="colorOverCh" type="checkbox" /> <?php echo __('Apply to all areas', 'canada-html5-map'); ?></label>
            <br />
            <?php
        }
        ...
        <span class="title"><?php echo __('Image URL:', 'canada-html5-map'); ?> </span>
            ...
            <span style="font-size: 10px; cursor: pointer;" onclick="clearImage(this)"><?php echo __('clear', 'canada-html5-map'); ?></span>
        <span class="tipsy-q" original-title="<?php esc_attr_e('The path to file of the image to display in a popup', 'canada-html5-map'); ?>">[?]</span><br />
        </div>
    ...
    <input type="hidden" name="act_type" value="canada-html5-map-states-save" />
