...
function wccs_admin_plugin_actions_pro($links) {
            $wccs_plugin_links = array(
                    '<a href="admin.php?page=woocommerce-checkout-manager-pro">'.__('Settings', 'woocommerce-checkout-manager-pro' ).'</a>',
                    '<a href="http://www.trottyzone.com/forums/forum/wooccm/">'.__('Support', 'woocommerce-checkout-manager-pro' ).'</a>',
                    );
...
function wooccm_check_duplicate_dir(){
	$number = 0;
	$free = 0;
	$dir = new DirectoryIterator(ABSPATH.'wp-content/plugins');
	foreach ($dir as $fileinfo) {
		if (!$fileinfo->isDot()) {
			if( stripos($fileinfo->getFilename(), 'woocommerce-checkout-manager-pro') !== false )   {
				$number = $number + 1;
			}
			if( $fileinfo->getFilename() == 'woocommerce-checkout-manager' )   {
				$free = 1;
			}
		}
	}
	return array($number, $free);
}

function wooccm_deactivate_plugin_conditional() {
	$name = 'woocommerce-checkout-manager-pro/woocommerce-checkout-manager-pro.php';
	$active = wooccm_check_duplicate_dir();
    if ( is_plugin_active( 'woocommerce-checkout-manager/woocommerce-checkout-manager.php' ) ) {
    	add_action('admin_notices', 'wooccm_admin_notice_previous');
	...
	if( $active[0] != 1 && !is_plugin_active( 'woocommerce-checkout-manager/woocommerce-checkout-manager.php' ) ) {
		add_action('admin_notices', 'wooccm_admin_notice_dupcheck');
    	deactivate_plugins( $name );
	}
	
	if( $active[1] == 1 && !is_plugin_active( 'woocommerce-checkout-manager/woocommerce-checkout-manager.php' ) ) {
		add_action('admin_notices', 'wooccm_admin_notice_freenotactive');
    	deactivate_plugins( $name );
	}
...
function wooccm_admin_notice_freenotactive() {
    echo '<div class="error"><p><strong>'.__( 'WooCheckout detected that the free version of the plugin is deactivated. Delete the plugin before activating.', 'woocommerce-checkout-manager-pro' ).'</strong></p></div>';
}

function wooccm_admin_notice_woo() {
    echo '<div class="error"><p><strong>'.__( 'WooCommerce is not active! WooCommerce Checkout Manager Pro requires WooCommerce to be active.', 'woocommerce-checkout-manager-pro' ).'</strong></p></div>';
}

function wooccm_admin_notice_previous() {
    echo '<div class="error"><p><strong>'.__( 'WooCheckout detected that the free version of the plugin is installed already. Deactivate and delete that plugin before activating.', 'woocommerce-checkout-manager-pro' ).'</strong></p></div>';
}

function wooccm_admin_notice_dupcheck() {
	$begin_div = '<div class="error"><p><strong>';
	$end_div = '</strong></p></div>';
	$alert1 = '<span style="color:red;">wp-content/plugins</span>';
	$alert2 = '<span style="color:red;">woocommerce-checkout-manager-pro</span>';
    echo sprintf(__('%s WooCheckout detected that your WordPress Plugin directory %s has more than one %s folder. Delete the other one then reactivate. %s', 'woocommerce-checkout-manager-pro') ,$begin_div,$alert1,$alert2,$end_div);
}

function wooccm_plugin_row_meta( $links, $file ) {
    if ( strpos( $file, 'woocommerce-checkout-manager-pro.php' ) !== false ) {

    	$row_meta = array(
        			'documentation'    =>  '<a href="http://www.trottyzone.com/woocommerce-checkout-manager-pro-documentation/" title="' . esc_attr( __( 'View WooCommerce Checkout Manager Pro Documentation', 'woocommerce' ) ) . '">' . __( 'Documentation', 'woocommerce-checkout-manager-pro' ) . '</a>',
      			);
...
$shipping = array( 'country', 'first_name', 'last_name', 'company', 'address_1', 'address_2', 'city', 'state', 'postcode' );
    ...
    if ( get_current_screen()->post_type == 'shop_order' ){
		...
		<?php
		if( empty($values) && empty($values2) && empty($values3) ) { ?>
			<option value=""><?php _e('No Added Fields', 'woocommerce-checkout-manager-pro'); ?></option>
		<?php }else { ?>
			<option value=""><?php _e('Field Name', 'woocommerce-checkout-manager-pro'); ?></option>
		<?php }
...
function display_notices_wooccm(){
	echo '<form method="post" name="clickhere" action=""><div id="setting-error-settings_updated" class="updated settings-error click-here-wooccm"><p><strong>'.__('Almost done! Install latest settings.','woocommerce-checkout-manager-pro').'</strong> | <input type="hidden" name="click-here-wooccm" value="y" /><input type="submit" class="button button-primary button-hero" value="Click Here" /></p></div></form>';
